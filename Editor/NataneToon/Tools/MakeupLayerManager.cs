    ...
    public class MakeupLayerManager : EditorWindow
    {
        ...
        private LayerData[] layers = new LayerData[]
        {
            ...
            new LayerData {
                ...
                displayName = "第5レイヤー 5th Layer"
            },
        };

        private string[] blendModeNames = new string[] { "加算 Add", "乗算 Multiply", "オーバーレイ Overlay", "スクリーン Screen" };
        ...
        [MenuItem("Tools/Natane/Makeup Layer Manager", false, 130)]
        public static void ShowWindow()
        {
            var window = GetWindow<MakeupLayerManager>("メイクアップレイヤー Makeup");
        ...
        private void DrawHeader()
        {
            ...
            EditorGUILayout.LabelField("メイクアップレイヤー管理 Makeup Layer Manager", EditorStyles.boldLabel);
            EditorGUILayout.LabelField("5つのテクスチャレイヤーをHSV調整付きで視覚的に管理\nManage 5 texture layers with HSV adjustment visually", EditorStyles.miniLabel);
        ...
        private void DrawMaterialSelection()
        {
            ...
            if (GUILayout.Button("選択中のマテリアルを使用 Use Selected Material", GUILayout.Height(25)))
            {
        ...
        {
            EditorGUILayout.BeginVertical(EditorStyles.helpBox);
            EditorGUILayout.LabelField("レイヤースタック Layer Stack", EditorStyles.boldLabel);
            ...
            EditorGUILayout.LabelField("ベースレイヤー Base Layer (_MainTex)", EditorStyles.boldLabel);
            ...
            if (targetMaterial.HasProperty("_MainTex"))
            {
                ...
                {
                    EditorGUILayout.ObjectField("テクスチャ Texture", targetMaterial.GetTexture("_MainTex"), typeof(Texture2D), false);
                }
            }
            EditorGUILayout.EndVertical();
        ...
        private void DrawLayer(LayerData layer, int index)
        {
            ...
            if (hasTexture)
            {
                if (GUILayout.Button("クリア Clear", GUILayout.Width(80)))
                {
                    if (EditorUtility.DisplayDialog(
                        "レイヤーをクリア Clear Layer",
                        $"{layer.displayName}のテクスチャをクリアしますか？\nClear texture for {layer.displayName}?",
                        "はい Yes",
                        "いいえ No"))
                    {
            ...
            if (layer.foldout)
            {
                ...
                if (hasTexture)
                {
                    ...
                    {
                        EditorGUI.BeginChangeCheck();
                        Color color = EditorGUILayout.ColorField("色 Color Tint", targetMaterial.GetColor(layer.colorProp));
                        if (EditorGUI.EndChangeCheck())
                        {
                    ...
                    // HSV adjustment
                    EditorGUILayout.Space(5);
                    EditorGUILayout.LabelField("HSV調整 HSV Adjustment", EditorStyles.boldLabel);

                    if (targetMaterial.HasProperty(layer.hueProp))
                    {
                        EditorGUI.BeginChangeCheck();
                        float hue = EditorGUILayout.Slider("色相 Hue", targetMaterial.GetFloat(layer.hueProp), -180f, 180f);
                        if (EditorGUI.EndChangeCheck())
                        {
                    ...
                    {
                        EditorGUI.BeginChangeCheck();
                        float saturation = EditorGUILayout.Slider("彩度 Saturation", targetMaterial.GetFloat(layer.saturationProp), 0f, 2f);
                        if (EditorGUI.EndChangeCheck())
                        {
                    ...
                    {
                        EditorGUI.BeginChangeCheck();
                        float value = EditorGUILayout.Slider("明度 Value", targetMaterial.GetFloat(layer.valueProp), 0f, 2f);
                        if (EditorGUI.EndChangeCheck())
                        {
                    ...
                    if (targetMaterial.HasProperty(layer.blendModeProp))
                    {
                        ...
                        blendMode = EditorGUILayout.Popup("ブレンドモード Blend Mode", blendMode, blendModeNames);
                        if (EditorGUI.EndChangeCheck())
                        {
                    ...
                    // Reset button
                    EditorGUILayout.Space(5);
                    if (GUILayout.Button("このレイヤーをリセット Reset This Layer"))
                    {
                        ResetLayer(layer);
                    }
                }
                ...
                {
                    EditorGUILayout.HelpBox("テクスチャが設定されていません\nNo texture assigned", MessageType.Info);
                }
        ...
        {
            EditorGUILayout.BeginVertical(EditorStyles.helpBox);
            EditorGUILayout.LabelField("レイヤーテンプレート Layer Templates", EditorStyles.boldLabel);
            EditorGUILayout.HelpBox(
                "事前設定されたHSV値でレイヤーを素早く設定\n" +
                "Quickly configure layers with preset HSV values",
                MessageType.Info);

            selectedTemplate = (LayerTemplate)EditorGUILayout.EnumPopup("テンプレート Template", selectedTemplate);

            EditorGUILayout.BeginHorizontal();

            if (GUILayout.Button("第2レイヤーに適用 Apply to 2nd", GUILayout.Height(25)))
            {
                ApplyTemplate(0, selectedTemplate);
            }
            if (GUILayout.Button("第3レイヤーに適用 Apply to 3rd", GUILayout.Height(25)))
            {
            ...
            if (GUILayout.Button("第4レイヤーに適用 Apply to 4th", GUILayout.Height(25)))
            {
                ApplyTemplate(2, selectedTemplate);
            }
            if (GUILayout.Button("第5レイヤーに適用 Apply to 5th", GUILayout.Height(25)))
            {
        ...
        private void DrawBatchOperations()
        {
            ...
            if (GUILayout.Button("すべてリセット Reset All", GUILayout.Height(30)))
            {
                if (EditorUtility.DisplayDialog(
                    "すべてリセット Reset All",
                    "すべてのメイクアップレイヤーをリセットしますか？\nReset all makeup layers?",
                    "はい Yes",
                    "いいえ No"))
                {
            ...
            if (GUILayout.Button("すべてクリア Clear All", GUILayout.Height(30)))
            {
                if (EditorUtility.DisplayDialog(
                    "すべてクリア Clear All",
                    "すべてのメイクアップレイヤーのテクスチャをクリアしますか?\nClear all makeup layer textures?",
                    "はい Yes",
                    "いいえ No"))
                {
        ...
        private void ApplyTemplate(int layerIndex, LayerTemplate template)
        {
            ...
            EditorUtility.DisplayDialog(
                "テンプレート適用 Template Applied",
                $"{template}テンプレートを{layer.displayName}に適用しました\nApplied {template} template to {layer.displayName}",
                "OK");
        }
    }
}
