...
- **🐛 v1.0.2のシェーダーエラーを修正**
  - テッセレーション機能を削除
  - 「Did not find shader kernel 'hull/domain/tessvert' to compile」エラーを解消
  - シェーダーがUnityで正常にコンパイルされるように修正

#### 削除された機能
- ❌ テッセレーション（Tessellation）機能
  - Unityのシェーダーシステムの制約により、現在の実装方法では動作しないため削除
  - 将来的に別の方法で再実装を検討

#### 影響
- v1.0.2でテッセレーション機能を使用していた場合、v1.0.3にアップデートすると機能が削除されます
- それ以外のすべての機能（VRC Light Volumes、日本語UI等）は正常に動作します
- v1.0.1からのアップデートは問題ありません

#### 推奨事項
- **v1.0.2を使用中の方**: v1.0.3へのアップデートを推奨（シェーダーエラー解消）
- **v1.0.1を使用中の方**: v1.0.3へのアップデートは任意（安定版として推奨）
- **v1.0.0を使用中の方**: v1.0.3へのアップデートを推奨（VRC Light Volumes対応）

### v1.0.2 (2025-10-31) - ⚠️ 非推奨
**テッセレーション機能を含むが、重大なシェーダーエラーがあります。v1.0.3を使用してください。**
