...
half4 frag(v2f i) : SV_Target
{
    ...
    // ===== Color Preservation (Applied before effects) =====
    // This preserves texture colors while allowing lighting to brighten the image
    // Applied here so that effects (Specular, SSS, Rim, etc.) are still added on top

    // 1. Texture Color Preservation (Albedo Preservation)
    // Preserve texture color while maintaining shadow and lighting information
    if (_AlbedoPreservation > 0.001)
    {
        // Calculate luminance (brightness) from lighting
        float litLuminance = dot(col.rgb, float3(0.299, 0.587, 0.114));
        float originalLuminance = dot(originalAlbedo, float3(0.299, 0.587, 0.114));

        // Calculate lighting factor (how much darker/brighter compared to original)
        // Allow up to 1.3x brightness to permit brightening without harsh cutoff
        float lightingFactor = min(litLuminance / (originalLuminance + 0.001), 1.3);

        // Apply lighting factor to original albedo
        // This preserves color while maintaining shadow darkness
        float3 preservedColor = originalAlbedo * lightingFactor;

        // Gentle clamp to prevent harsh white cutoff (allow slight overshoot)
        preservedColor = min(preservedColor, float3(1.05, 1.05, 1.05));

        // Blend between lit color and preserved color
        col.rgb = lerp(col.rgb, preservedColor, _AlbedoPreservation);
    }
