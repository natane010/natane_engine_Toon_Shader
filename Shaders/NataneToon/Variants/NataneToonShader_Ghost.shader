// Natane Toon Shader - Ghost Variant
// 幽霊表現: 深度プリパス + ZTest Equal により、自己重なりの二重ブレンドや
// 裏面の透けが構造的に発生しない単層半透明レンダリング。
// (GrabPassなし・ShadowCasterなし・アウトラインパスなし)
Shader "Natane/Toon Shader (Ghost)"
{
    Properties
    {
        // ===== Base Settings (基本設定) =====
        ...
        [Header(Ghost)]
        [HDR] _GhostRimColor ("Ghost Rim Color", Color) = (0.4, 0.8, 1.2, 1)
        _GhostFresnelAlpha ("Ghost Fresnel Alpha (center fade)", Range(0, 1)) = 0.7
        _GhostFresnelPower ("Ghost Fresnel Power", Range(0.5, 8)) = 2.5
        _GhostRimStrength ("Ghost Rim Strength", Range(0, 2)) = 1
        _GhostDepthCutoff ("Ghost Depth Cutoff", Range(0, 0.5)) = 0.02
        [Space(10)]
    ...
    SubShader
    {
        ...
        // Ghost depth prepass: writes depth only. The forward passes render at
        // ZTest Equal, so exactly one (the nearest) surface layer is blended —
        // no self-overlap double-blend, no backface bleed-through.
        // Uses the same vert (same keywords) so vertex deformation features
        // (VAT/Smear/PerspectiveFlat/FaceOrtho) stay depth-consistent.
        Pass
        {
            Name "GHOST_DEPTH_PREPASS"
            ...
            #pragma target 3.5
            #pragma vertex vert
            #pragma fragment fragGhostDepth
            ...
            fixed4 fragGhostDepth(v2f i) : SV_Target
            {
                UNITY_SETUP_STEREO_EYE_INDEX_POST_VERTEX(i);
                // Keep depth in sync with the mirror/camera visibility feature.
                ...
                #endif
                half ghostAlpha = tex2D(_MainTex, i.uv).a * _Color.a;
                clip(ghostAlpha - _GhostDepthCutoff);
                return 0;
            }
            ENDCG
        }
