...
Shader "Natane/Toon Shader (Fur)"
{
    Properties
    {
        // <auto-generated:natane-properties> 手編集しないでください。Tools > Natane > ビルド最適化 から再生成します。
        // ===== Base Settings (基本設定) =====
        [Header(Main Texture)]
        ...
        _FurFluff ("Fur Fluffiness", Range(0, 1)) = 0.5
        _FurRootOffset ("Fur Root Offset", Range(-1, 0)) = -0.3
        [Toggle] _FurUseNoiseTex ("Fur Use Noise Texture", Float) = 0
        ...
        // Fur Method (シェル法 / フィン法の使い分け)
        [Enum(Shell,0,Fin,1,ShellAndFin,2)] _FurMethod ("Fur Method", Float) = 0
        _FurFinViewThreshold ("Fin View Threshold", Range(0.05, 1)) = 0.75
        _FurFinJoints ("Fin Joints", Range(1, 4)) = 2
        _FurFinNormalBlend ("Fin Normal Blend", Range(0, 1)) = 0.5
        _FurFinRandomDir ("Fin Direction Randomness", Range(0, 1)) = 0.2

        // </auto-generated:natane-properties>
    }
...
#pragma shader_feature_local _SOFT_LIGHTING_MODE
        ...
        Pass
        {
            ...
            float _FurLODDistance; float _FurLODMinLayers; float _FurMethod; float4 _FurNoiseTex_ST;
            float _FurFluff; float _FurRootOffset; float _FurUseNoiseTex;
        ...
        // --- Fur Fin (silhouette cards, geometry shader) ---
        // Shells thin out exactly where you look along them; fins cover that case.
        // PC only: the pass requires geometry shader support and is skipped where the
        // platform does not have it (#pragma require geometry).
        Pass
        {
            Name "FUR_FIN"
            Tags { "LightMode" = "Always" }
            ...
            CGPROGRAM
            #pragma vertex finVert
            #pragma geometry finGeom
            #pragma fragment finFrag
            #pragma target 4.0
            ...
            float _FurLODDistance; float _FurLODMinLayers; float _FurMethod; float4 _FurNoiseTex_ST;
            float _FurFluff; float _FurRootOffset; float _FurUseNoiseTex;
            float _FurFinViewThreshold; float _FurFinJoints; float _FurFinNormalBlend;
            float _FurFinRandomDir;
            sampler2D _FurNoiseTex; sampler2D _FurMask;
            #ifdef _FUR
            #include "../Include/Rendering/NataneToonFurFin.hlsl"
            #else
            struct appdata_fin_stub { float4 vertex : POSITION; };
            struct v2g_fin_stub { float4 pos : SV_POSITION; };
            v2g_fin_stub finVert(appdata_fin_stub v) { v2g_fin_stub o; o.pos = float4(0,0,0,1); return o; }
            [maxvertexcount(3)]
            void finGeom(triangle v2g_fin_stub i[3], inout TriangleStream<v2g_fin_stub> stream) { }
            fixed4 finFrag(v2g_fin_stub i) : SV_Target { clip(-1); return 0; }
            #endif
            ENDCG
        }
